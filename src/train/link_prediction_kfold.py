"""
K-Fold Cross Validation for Multi-Task Learning (MTL) training on small datasets.

This version uses k-fold cross validation to make better use of limited data
and provide more robust evaluation metrics. The core logic is identical to
link_prediction.py - only the data splitting is different.
"""
...
def train_kfold(
    ...
    num_heads: int = 8,
    dropout: float = 0.2,
    save_path: str = "data/models/linkpred_gat_kfold.pt",
    # Sampler params
    ...
    # Get the target edge indices for k-fold splitting
    target_edges = full_data[target_edge_type].edge_index.t()
    
    # Create k-fold splits
    kf = KFold(n_splits=n_folds, shuffle=True, random_state=42)
    
    all_fold_metrics = []
    best_fold_auc = -math.inf
    best_fold_state = None
    best_fold_idx = -1

    device_t, rank, world_size, local_rank = ddp_setup()

    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(target_edges)):
        ...
        # Optional loader for target edge type using neighbor sampling (same as original logic)
        loader = None
        if use_sampler:
            # Concatenate pos/neg indices for training label set
            pos_idx = fold_data[target_edge_type]["train_edge_label_index_pos"]
            neg_idx = fold_data[target_edge_type]["train_edge_label_index_neg"]
        ...
        # Training loop (identical to original logic)
        for epoch in range(1, epochs_per_fold + 1):
            model.train()

            # Extract edge attributes for model forward pass
            edge_attr_dict = {}
            for edge_type in fold_data.edge_types:
                if hasattr(fold_data[edge_type], "edge_attr") and fold_data[edge_type].edge_attr is not None:
                    edge_attr_dict[edge_type] = fold_data[edge_type].edge_attr.to(device_t)
            
            # Initialize node features
            num_nodes_by_type = {nt: fold_data[nt].num_nodes for nt in fold_data.node_types}
            ...
            z_dict = model(x_dict, fold_data.edge_index_dict, edge_attr_dict)
            ...
            if is_main_process(rank) and epoch % 10 == 0:
                print(
                    f"Fold {fold_idx + 1} | Epoch {epoch:03d} | lp_loss {avg_lp_loss:.4f} | closs {closs_val:.4f} | "
    ...
    # Save best model
    if best_fold_state is not None and is_main_process(rank):
        torch.save({
        ...
        avg_aucs = [fold["best_val_auc"] for fold in all_fold_metrics]
        print(f"Average AUC across folds: {np.mean(avg_aucs):.4f} ± {np.std(avg_aucs):.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs-per-fold", type=int, default=50, help="Epochs to train per fold")
    ...
    parser.add_argument("--out", type=str, default="data/models/linkpred_gat_kfold.pt")
    ...
    parser.add_argument("--contrastive-pos-threshold", type=float, default=0.5)
    ...
    target_et = parse_edge_types([args.target_edge_type])[0]
    train_kfold(
